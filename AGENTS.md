...
3. **Slash command scripts live under `plugins/cursor/scripts/<cmd>.mjs`.** Direct Bash wrappers at `plugins/cursor/commands/<cmd>.md` must use `node "${CLAUDE_PLUGIN_ROOT}/scripts/<cmd>.mjs" -- "$ARGUMENTS"` with quoted `$ARGUMENTS` — unquoted breaks under zsh on any prompt containing `?`, `*`, or `@`. `/cursor:rescue` is the only exception: it routes through the `cursor-rescue` subagent so the `Agent` tool stays in scope.
4. **`Bash(node:*)` is the default permission pattern used in `allowed-tools`.** The only exception is `/cursor:rescue`, which may also use `AskUserQuestion` and `Agent` so it can route through the `cursor-rescue` subagent like the Codex plugin. Do not invent path-based patterns — Claude Code does not expand `${CLAUDE_PLUGIN_ROOT}` inside `allowed-tools`.
